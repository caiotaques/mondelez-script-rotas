...
loc_df = pd.read_csv('input/end_pdvs_coords.csv', sep=';')
...
print(f"loc_df cols: {loc_df.columns.tolist()}\n df_hierarquia cols: {df_hierarquia.columns.tolist()}")
...
# ============ Criar df_gus (hierarquia + loc filtrado) ==============
...
print(
    f"[FILTER] pdvs_total={pdvs_total} -> pdvs_gus={pdvs_gus} ({_pct(pdvs_gus, pdvs_total):.1f}%) | "
...
df_merge["id_vendedor"] = vd[0].str.strip()
...
df2["_kpi"] = (
    df2['Kpi'].astype(str).str.strip().str.replace(r"\W+", "_", regex=True)
)
...
if "Datapesquisa"  in df2.columns:
    df2["Datapesquisa" ] = pd.to_datetime(df2["Datapesquisa" ], errors="coerce", dayfirst=False)
    df2 = (df2.sort_values("Datapesquisa" )
               .drop_duplicates(subset=id_cols + ["_kpi"], keep="last"))
...
print(f"[SCORES] df_merge estrelas_total nulls={null_stars}/{len(df_merge)} ({_pct(null_stars, len(df_merge)):.1f}%)")

# ============ Ingerir SELLOUT (parquet) e criar sellout_ton_ytd + trend_l6m ============
t_sell = time.time()

sell_path = "input/osa_p01_p08_25.parquet"
...
# logs úteis
print(f"[SELLOUT] cnpjs no parquet={sell_feats['cnpj'].nunique()} | linhas={len(sell_feats)}")
print(
    f"[SELLOUT] sellout_ton_ytd stats: "
    f"min={sell_feats['sellout_ton_ytd'].min():.4f} "
    f"p50={sell_feats['sellout_ton_ytd'].median():.4f} "
    f"p90={sell_feats['sellout_ton_ytd'].quantile(0.9):.4f} "
    f"max={sell_feats['sellout_ton_ytd'].max():.4f}"
)
...
print(f"[SELLOUT] df_merge sellout_ton_ytd nulls={null_sell}/{len(df_merge)} ({_pct(null_sell, len(df_merge)):.1f}%)")
print(f"[SELLOUT] df_merge trend_l6m nulls={null_trnd}/{len(df_merge)} ({_pct(null_trnd, len(df_merge)):.1f}%)")

print(f"[SELLOUT] elapsed_sec={time.time() - t_sell:.1f}")
...
print(f"[PREP] elapsed_sec={time.time() - t0:.1f}")
