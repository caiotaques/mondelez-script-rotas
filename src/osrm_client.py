...
def build_matrices_with_osrm_or_fallback(pts_latlon, route_inflation=1.25, avg_speed_kmph=30.0,
                                        prefer_osrm=True, force_haversine=False,
                                        return_method=False, verbose=True):
    """
    Tenta OSRM (se prefer_osrm and not force_haversine). Se falhar, usa haversine + inflação.
    Parâmetros:
      - pts_latlon: array [[lat, lon], ...]
      - route_inflation: fator aplicado ao haversine (fallback)
      - avg_speed_kmph: velocidade média para estimar tempo no fallback
      - prefer_osrm: se True, tenta OSRM primeiro; se False, usa fallback direto
      - force_haversine: se True, força usar fallback mesmo que OSRM esteja disponível
      - return_method: se True, retorna também a string do método usado: "osrm" ou "haversine"
      - verbose: se True, imprime qual método foi usado
    Retorna:
      - (Dkm, Tmin) ou (Dkm, Tmin, method) se return_method=True
    """
    import numpy as np
    method_used = None
    ...
    if force_haversine:
        if verbose:
            print("[osrm_client] force_haversine=True -> usando haversine fallback")
    else:
        if prefer_osrm:
            try:
                Dkm, Tmin = osrm_table_full(pts_latlon)
                method_used = "osrm"
                # Se o OSRM não devolveu distance, recomputamos Dkm por dur * v
                if not np.isfinite(Dkm).any():
                    Dkm = (Tmin.astype(float) / 60.0) * (avg_speed_kmph)  # aproxima
                if verbose:
                    print("[osrm_client] método usado: OSRM")
                if return_method:
                    return Dkm, Tmin, method_used
                return Dkm, Tmin
            except Exception:
                if verbose:
                    print("[osrm_client] falha no OSRM, caindo para haversine fallback")
                # cairá para cálculo por haversine abaixo
    ...
    if verbose:
        print("[osrm_client] método usado: Haversine (fallback)")
    if return_method:
        return Dkm, Tmin, method_used
    return Dkm, Tmin