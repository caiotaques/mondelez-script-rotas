...
def rotas(periodo_pesquisa: str, periodo_rotas: str) -> None:
    ...
    df_all_rotas = df_all_rotas.rename(columns={
        'ordem': 'nsequence'
    })



    # periodo_rotas é algo como "p03", extraímos o mês
    mes_rotas = int(periodo_rotas[1:])
    ano_atual = pd.Timestamp.now().year
    dstart = pd.Timestamp(year=ano_atual, month=mes_rotas, day=1).strftime('%Y-%m-%d')
    dend = (pd.Timestamp(year=ano_atual, month=mes_rotas, day=1) + pd.offsets.MonthEnd(0)).strftime('%Y-%m-%d')
    ...
    df_all_rotas['drecurrenceend'] = dend
    df_all_rotas['cidcustomer'] = df_all_rotas['cod_cliente'] # não sei ainda
    # adicionar um zero á esquerda de cidcustomer, por particularidade do sistema
    df_all_rotas['cidcustomer'] = df_all_rotas['cidcustomer']
    df_all_rotas['cweekday'] = '1111111'
    df_all_rotas['cweekmonth'] = '111111'
    df_all_rotas['mc1enabled'] = 1
    # df_all_rotas['cidterritory'] = '0202045' # 020202-CRISTINA VAZ (base hierarquia)
    df_all_rotas['nfixedday'] = 0
    df_all_rotas['nrecurrence'] = 0
    df_all_rotas['cdetail'] = 3
    df_all_rotas['cprofiletarget'] = 'salesman'
...
if __name__ == "__main__":
    rotas(periodo_rotas="P03", periodo_pesquisa="P02")